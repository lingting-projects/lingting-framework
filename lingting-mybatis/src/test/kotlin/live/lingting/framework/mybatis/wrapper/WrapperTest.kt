...
internal class WrapperTest {
    ...
    fun test() {
        val query = query<Any>()
            ...
            .`in`("i1", "iv1", "iv2")
            .between("b1", "bv1", "bv2")
            .and(Consumer { m: QueryWrapper<Any> -> m.eq("aq1", "aqv1").and(Consumer { mm: QueryWrapper<Any> -> mm.eq("aaq1", "aav1") }) })
            .and(Consumer { m: QueryWrapper<Any> -> m.eq("oq1", "oqv1").or(Consumer { mm: QueryWrapper<Any> -> mm.eq("ooq1", "oov1") }) })
            .groupBy("g1", "g2")
            .orderByDesc("od1", "od2")
            .orderByAsc("oa1", "oa2")
            .having("sum(t.h1) > 0")
        ...
        Assertions.assertEquals("t.s2, t.s1, t.s3", query.getSqlSelect())
    ...
    fun testO() {
        val query: QueryWrapper<Table> = query<Table>(Table::class.java)
            ...
            .eq("e1", "ev1")
            .`in`<Table>("i1",
                Consumer { queryWrapper: QueryWrapper<Table> ->
